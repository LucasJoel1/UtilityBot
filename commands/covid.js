...
exports.run = async (bot, message, [country]) => {
    try {
        ...
        const options1 = await fetch(`https://disease.sh/v3/covid-19/countries/${country}`);
        
        const options2 = await fetch(`https://disease.sh/v3/covid-19/vaccine/coverage/countries/${country}?lastdays=1&fullData=true`)
